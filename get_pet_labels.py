...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
        ...
        # Split the filename by '_' to separtate teh 
        # and convert each word to lowercase
        filename_parts = in_files[idx].lower().split("_")
        for word in filename_parts:
            # Removes digits from words
            if word.isalpha():
                pet_label += word + " "
        
        # Strip leading/trailing whitespace from the pet label
        pet_label = pet_label.strip()
        
        # If filename doesn't already exist in the dictionary, add it and its
        # pet label; otherwise, print an error message indicating duplicate files
